 ...
 */
export async function checkCourierServiceability(orderId) {
  const token = await getShiprocketToken();
  const query = new URLSearchParams({
    order_id: orderId
  }).toString();

  const response = await fetch(`${SHIPROCKET_BASE_URL}/courier/serviceability/?${query}`, {
    method: "GET",
    headers: {
      Authorization: `Bearer ${token}`
    }
  ...
  const allCouriers = data.data?.available_courier_companies || [];
  
  // Add the additional charges (whatsapp, coverage) to the base rate to match dashboard pricing
  const couriersWithRates = allCouriers.map(c => ({
    ...c,
    display_rate: (Number(c.rate) + Number(c.whatsapp_charges || 0) + Number(c.coverage_charges || 0)).toFixed(2)
  }));

  // Sort couriers by price (Cheapest first)
  return couriersWithRates.sort((a, b) => Number(a.display_rate) - Number(b.display_rate));
}
 ...
 */
export async function scheduleShiprocketPickup(shipment_id, courierId = null) {
  // 1. Assign Courier & Generate AWB
  const courierData = await assignCourier(shipment_id, courierId);
